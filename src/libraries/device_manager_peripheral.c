...
void dm_ble_evt_handler(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        {
            int rssi = 0;
            rssi = (int)(p_ble_evt->evt.gap_evt.params.rssi_changed.rssi);
            Update_RSSI(rssi);
            break;
        }	
        default:
...
#define THRESHOLD_WARNING_RSSI  (-70)
#define THRESHOLD_ALARM_RSSI    (-90)

void Update_RSSI(int rssi)
{
    int avg_rssi=0;
    static int latest_rssi[10] = {0};
    static unsigned char idx=0;
    unsigned char i;

    latest_rssi[idx] = rssi;

    for(i=0;i<SAMPLE_COUNT;i++)
    {
        avg_rssi += latest_rssi[(unsigned char)(idx-i)];
    }
    avg_rssi = avg_rssi/SAMPLE_COUNT;

    if(avg_rssi > THRESHOLD_WARNING_RSSI){
        led_off();
        //Everything is fine.
    } else if(avg_rssi>THRESHOLD_WARNING_RSSI){
        led_blink();
    }
    else{
        led_on();
    ...
    idx++;  //even if it is overflowed, it is still allowed.
}
